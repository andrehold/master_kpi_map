...
import { ChevronDown, ChevronUp, Search, RefreshCw, Filter, Download, Sun, Moon } from "lucide-react";
...
type KPIValueType =
  | "percent" | "ratio" | "bps" | "sigma" | "index" | "ivrank" | "ms" | "price" | "text";

interface KPIDef { id: string; name: string; description?: string; strategies: Strategy[]; valueType: KPIValueType; }
interface KPIGroup { id: string; title: string; kpis: KPIDef[]; }

const KPI_GROUPS: KPIGroup[] = [
  { id: "vol-skew", title: "1. Volatility & Skew Metrics", kpis: [
  ...
  { id: "rv-vs-iv", title: "2. Realized vs Implied (RV vs IV)", kpis: [
  ...
  { id: "regime-macro", title: "3. Market Regime & Macro Filters", kpis: [
  ...
  { id: "micro-flow", title: "4. Microstructure, Flows & Liquidity", kpis: [
  ...
  { id: "strategy-health", title: "5. Strategy-Specific Health Metrics", kpis: [
  ...
  { id: "execution-costs", title: "6. Execution & Cost KPIs", kpis: [
  ...
  { id: "risk-pnl", title: "7. Risk, Capital & P&L", kpis: [
  ...
  { id: "ops-process", title: "8. Operational & Process Health", kpis: [
...
function sampleFor(type: KPIValueType): string {
  switch (type) {
    ...
    case "text": {
      const isContango = Math.random()>0.35; const v = rand(0.0,8.0,1) * (isContango?1:-1);
...
const TOKENS = {
  light: { colorScheme: "light", bg: "#F6F8FC", surface950: "#FFFFFF", surface900:"#F8FAFF", border:"#E2E8F0", shadow:"0 1px 2px rgba(16,24,40,.04), 0 1px 1px rgba(16,24,40,.06)", fg:"#0F172A", fgMuted:"#64748B", brand400:"#60A5FA", brand500:"#2563EB", brand600:"#1D4ED8" },
  dark: { colorScheme: "dark", bg:"#0a0f1a", surface950:"#0b1020", surface900:"#0e1629", border:"#1e293b", shadow:"0 1px 0 rgba(255,255,255,0.04)", fg:"#e2e8f0", fgMuted:"#94a3b8", brand400:"#22d3ee", brand500:"#6366f1", brand600:"#4f46e5" }
} as const;
type ThemeKey = keyof typeof TOKENS;

function TokenStyles({ theme }: { theme: ThemeKey }) {
  const t = TOKENS[theme];
  const css = `:root{--color-scheme:${t.colorScheme};--bg:${t.bg};--surface-950:${t.surface950};--surface-900:${t.surface900};--border:${t.border};--shadow:${t.shadow};--fg:${t.fg};--fg-muted:${t.fgMuted};--brand-400:${t.brand400};--brand-500:${t.brand500};--brand-600:${t.brand600};--radius-lg:.5rem;--radius-xl:.75rem;--radius-2xl:1rem;}`;
...
function GroupHeader({ title, open, onToggle }:{title:string; open:boolean; onToggle:()=>void}){
  return (
    <button onClick={onToggle} className="w-full flex items-center justify-between gap-3 px-4 py-3 rounded-xl bg-[var(--surface-950)] border border-[var(--border)] hover:bg-[var(--surface-900)] shadow-[var(--shadow)]">
    ...
    <div className="group rounded-2xl border border-[var(--border)] bg-[var(--surface-950)] p-4 shadow-[var(--shadow)] hover:border-[var(--brand-500)]/30 transition">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          {kpi.description && <div className="text-xs text-[var(--fg-muted)] mt-0.5">{kpi.description}</div>}
        </div>
        <div className="text-right">
          ...
          <div className="text-[10px] text-[var(--fg-muted)] flex items-center gap-1 justify-end">
            <span className="inline-block h-1.5 w-1.5 rounded-full bg-[var(--fg-muted)]" /> sample
          </div>
        </div>
      </div>
      <div className="mt-3 flex flex-wrap gap-1.5">
        {kpi.strategies.map((s) => (
          <span key={s} className="px-2 py-0.5 rounded-full text-[10px] border border-[var(--border)] text-[var(--fg-muted)] bg-[var(--surface-900)]">{s}</span>
        ))}
      </div>
    </div>
  )
...
export default function App(){
  ...
  const [openGroups, setOpenGroups] = useState<Record<string, boolean>>(()=>{ const init:Record<string,boolean>={}; KPI_GROUPS.forEach(g=>init[g.id]=true); return init; });
  const [samples, setSamples] = useState<Record<string,string>>(()=>buildSamples());
  ...
  function toggleStrategy(s: Strategy){ setActiveStrategies(prev=> prev.includes(s) ? prev.filter(x=>x!==s) : [...prev,s]); }
  function toggleGroup(id:string){ setOpenGroups(prev=>({ ...prev, [id]: !prev[id] })); }
  function regenerate(){ setSamples(buildSamples()); }
  function exportJSON(){ const payload={ generated_at:new Date().toISOString(), strategies: STRATEGIES, groups: KPI_GROUPS.map(g=>({ id:g.id, title:g.title, kpis:g.kpis.map(k=>({ id:k.id, name:k.name, strategies:k.strategies, value: samples[k.id] })) })) }; const blob = new Blob([JSON.stringify(payload,null,2)], { type:'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href=url; a.download=`master-kpi-map-sample-${Date.now()}.json`; a.click(); URL.revokeObjectURL(url); }
        ...
        <div className="mx-auto max-w-7xl px-4 py-3 flex items-center gap-3 justify-between">
          ...
          <div className="flex items-center gap-2">
            <div className="hidden md:flex items-center gap-3 text-xs text-[var(--fg-muted)]">
              <span className="px-2 py-1 rounded-lg bg-[var(--surface-900)] border border-[var(--border)] shadow-[var(--shadow)]">KPIs <span className="font-mono">{visibleKpis}</span>/<span className="font-mono">{totalKpis}</span></span>
              <span className="px-2 py-1 rounded-lg bg-[var(--surface-900)] border border-[var(--border)] shadow-[var(--shadow)]">Groups <span className="font-mono">{filteredKpis => null}</span></span>
            </div>
        ...
        <div className="grid md:grid-cols-3 gap-3 mb-6">
          <div className="md:col-span-2">
            <div className="flex items-center gap-2 w-full rounded-xl border border-[var(--border)] bg-[var(--surface-950)] px-3 py-2 shadow-[var(--shadow)]">
              ...
              <input value={search} onChange={(e)=> setSearch(e.target.value)} className="w-full outline-none text-sm placeholder:text-[var(--fg-muted)] bg-transparent text-[var(--fg)]" placeholder="Search KPI name or id (e.g., 'iv-rv-spread')" />
            </div>
          </div>
          <div>
            ...
            <div className="mt-2 flex flex-wrap gap-1.5">
              {STRATEGIES.map((s)=>(
                <StrategyTag key={s} label={s} active={activeStrategies.includes(s)} onClick={()=> setActiveStrategies(prev=> prev.includes(s)? prev.filter(x=>x!==s):[...prev,s])} />
              ))}
            </div>
          </div>
        </div>
            ...
            <section key={group.id}>
              ...
              {openGroups[group.id] && (
                <div className="mt-3 grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {group.kpis.map(kpi => (<KpiCard key={kpi.id} kpi={kpi} value={samples[kpi.id]} />))}
                </div>
              )}
            </section>
          ))}
          {filteredGroups.length === 0 && <div className="text-center py-16 text-[var(--fg-muted)]">No KPIs match your search/filters.</div>}
        </div>

        <div className="text-xs text-[var(--fg-muted)] mt-10">Samples are mock values for layout/dev only. Replace with live feeds from your data layer / APIs.</div>
      </main>
    </div>
  )
}
