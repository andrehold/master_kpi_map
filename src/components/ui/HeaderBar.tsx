import { RefreshCw, Download, Sun, Moon, Cloud } from "lucide-react";
      ...
      <div className="mx-auto max-w-7xl px-4 py-3 flex items-center gap-3 justify-between">
        <div className="flex items-center gap-3">
          <div className="h-9 w-9 rounded-xl bg-gradient-to-br from-[var(--brand-600)] to-[var(--brand-400)] text-white grid place-items-center shadow-[var(--shadow)]">
            <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M12 3l8 4-8 4-8-4 8-4Z"/><path d="M4 11l8 4 8-4"/><path d="M4 17l8 4 8-4"/>
            </svg>
          </div>
          <div>
        ...
        <div className="flex items-center gap-2">
          <div className="hidden md:flex items-center gap-3 text-xs text-[var(--fg-muted)]">
            ...
            <span className="px-2 py-1 rounded-lg bg-[var(--surface-900)] border border-[var(--border)] shadow-[var(--shadow)]">
              Groups <span className="font-mono">{groupsCount}</span>/8
            </span>
            {indexPrice != null && (
              <span
                className="px-2 py-1 rounded-lg bg-[var(--surface-900)] border border-[var(--border)] shadow-[var(--shadow)]"
                title={indexTs ? `As of ${new Date(indexTs).toLocaleTimeString()}` : undefined}
              >
                BTC <span className="font-mono">{`$${Math.round(indexPrice).toLocaleString()}`}</span>
              </span>
            )}
            {errorText && (
          ...
          <button onClick={onRegenerate} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-[var(--border)] bg-[var(--surface-900)] hover:bg-[var(--surface-950)] text-sm shadow-[var(--shadow)]">
          ...
          <button onClick={onExportJSON} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-[var(--border)] bg-[var(--surface-900)] hover:bg-[var(--surface-950)] text-sm shadow-[var(--shadow)]">
